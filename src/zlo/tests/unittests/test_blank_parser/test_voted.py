...
class WhenVotedParsed(BlankParserMixin):
    ...
    def because_we_parse_game_info(self):
        self.voted = self.blank_parser.parse_voted_list()

    def it_should_get_proper_values(self):
        expect(self.voted.voted_slots[0][0]).to(equal(1))
        expect(self.voted.voted_slots[1][0]).to(equal(4))
        expect(self.voted.voted_slots[2][0]).to(equal(5))
        expect(self.voted.voted_slots[3][0]).to(equal(7))
...
class WhenVotedHaveEmptyValues(BlankParserMixin):
    ...
    def because_we_parse_game_info(self):
        self.voted_event = self.blank_parser.parse_voted_list()
...
class WhenVotedHaveMultipleValuesInOneCircle(BlankParserMixin):
    ...
    def it_should_correct_parse_first_vote(self):
        expect(self.voted_event.voted_slots[0]).to(have_len(2))
        expect(self.voted_event.voted_slots[0][0]).to(equal(1))
        expect(self.voted_event.voted_slots[0][1]).to(equal(2))

    def it_should_correct_parse_second_vote(self):
        expect(self.voted_event.voted_slots[1]).to(have_len(1))
        expect(self.voted_event.voted_slots[1][0]).to(equal(3))
...
class WhenVotedHaveEmptyValuesWithSuccessfulVotes(BlankParserMixin):
    ...
    def it_should_correct_parse_0_round_votes(self):
        expect(self.voted_event.voted_slots[1]).to(have_len(1))
        expect(self.voted_event.voted_slots[0][0]).to(equal(0))
    ...
    def it_should_correct_parse_2_round_votes(self):
        ...
        expect(self.voted_event.voted_slots[2][0]).to(equal(5))

    def it_should_correct_parse_3_round_votes(self):
        expect(self.voted_event.voted_slots[3]).to(have_len(1))
        expect(self.voted_event.voted_slots[3][0]).to(equal(6))

    def it_should_correct_parse_4_round_votes(self):
        expect(self.voted_event.voted_slots[4][0]).to(equal(0))
